...
export default function DesignNew() {
  ...
  const handleSelectCustomer = (customer) => {
    ...
    navigate('/panel/designs/new/editor', {
      state: {
  ...
  const handleAddCustomer = async (e) => {
    ...
    try {
      ...
      const { data, error: insertError } = await supabase
        .from('customers')
        .insert([{ ...newCustomer, tenant_id: tenantId }])
        .select();
  ...
  const handleSubmitDesign = async (e) => {
    ...
    try {
      ...
      if (!supabase) {
        ...
        navigate(`/panel/designs/${mockDesign.id}`, { state: { successMessage: 'Tasarım başarıyla oluşturuldu!' } });
        return;
      }
      ...
      navigate(`/panel/designs/${newDesign.id}`, { state: { successMessage: 'Tasarım başarıyla oluşturuldu!' } });
    } catch (err) {
